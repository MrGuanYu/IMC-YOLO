...
def get_params():
    params = {
        'weight': r'D:\program\python\ultralytics_withV9\runs\detect\yolov8n-head-tcamb-myaifi\weights\best.pt',  # 训练出来的权重文件
        'cfg': r'D:\program\python\ultralytics_withV9\ultralytics\cfg\models\v8\yolov8n-head-tcamb-myaifi.yaml',  # 训练权重对应的yaml配置文件
        'device': 'cuda:0',
        'method': 'GradCAM',  # GradCAMPlusPlus, GradCAM, XGradCAM , 使用的热力图库文件不同的效果不一样可以多尝试
        'layer': 'model.model[19]',  # 想要检测的对应层
...
if __name__ == '__main__':
    ...
    model(r'D:\program\python\ultralytics_withV9\myDatasets\datasets\700hole_enhence_mix\test\images\IMG_20240310_151543_left_flip.jpg',
          r'D:\program\python\ultralytics_withV9\runs\heatmap')  # 第一个是检测的文件, 第二个是保存的路径